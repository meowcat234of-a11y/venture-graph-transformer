...
st.caption("Rank company momentum from relationship centrality, talent flow, and analyst sentiment.")
...
if uploaded is None:
    ...
    st.info("Showing a small demo graph. Upload a CSV with source, target, and optional weight columns to analyze your own data.")
else:
...
scores = {
    company: calculate_momentum(centrality[company], flows.get(company, {}).get("net_flow", 0.0), sentiment)
